...
module.exports = function () {
  ...
  function getPropertyName(node) {
    if (!node.is('declaration')) return; // TODO: Check that it's not a variable
  ...
  return {
    ...
    process: function (ast) {
      ast.traverseByType('block', function (node) {
        ...
        if (oneline && ast.syntax !== 'sass') return; // Update nodes
        ...
        walk({
          ...
          payload: function (info, i) {
            ...
            if (!whitespaceNode) return; // If it's a comment, insert an empty space node:
      ...
      ast.traverseByType('block', function (node) {
        ...
        var getResult = function (options) {
          let node = options.node,
              ...
              info = options.info,
              i = options.i;
          var prop = info.baseName; // If this is the last item in a row and we have a result,
          // then catch it
